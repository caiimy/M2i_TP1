...
if ! [ -f hosts ]; then
  ...
  echo $IP_WP ansible_ssh_user=future-oasis-399813 ansible_connection=ssh ansible_private_key_file=~/.ssh/id_rsa >> hosts
  echo "[db]" >> hosts
  echo $IP_DB ansible_ssh_user=future-oasis-399813 ansible_connection=ssh ansible_private_key_file=~/.ssh/id_rsa >> hosts
fi
cd ..
